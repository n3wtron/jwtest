...
public class ResultPage extends LayoutPage {
	...
	public ResultPage(final PageParameters params) {
		...
		final Result res = sesMapper.getMapper().get(BigInteger.valueOf(params.get("idResult").toLong()));

		add(new BookmarkablePageLinkSecure<String>("addRecycleLnk", AddResultPage.class, new PageParameters().add("idParent", res.getId()), Roles.ADMIN, "PROJECT_ADMIN", "MANAGER") {
			private static final long serialVersionUID = 1L;
		...
		BookmarkablePageLink<String> testcasePageLnk = new BookmarkablePageLink<String>("testcasePageLnk", TestCasePage.class, new PageParameters().add("idTest", res.getId_testcase()));
		testcasePageLnk.add(new Label("testcaseName", res.getTestCase().toString()));
		add(testcasePageLnk);

		BookmarkablePageLink<String> sesPageLnk = new BookmarkablePageLink<String>("sesPageLnk", SessionsPage.class, new PageParameters().add("idSession", res.getId_session()));
		...
		if (res.getId_parent() != null) {
			parentLnk = new BookmarkablePageLink<String>("parentLnk", ResultPage.class, new PageParameters().add("idResult", res.getId_parent()));
